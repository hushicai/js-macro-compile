...
function findMacroInCondition(condition, result) {
    // 二元运算
    if (condition instanceof ug.AST_Binary) {
        findMacroInCondition(condition.left, result);
        findMacroInCondition(condition.right, result);
        result.push(condition.operator);
    }
    else if (condition instanceof ug.AST_Unary) {
        // 一元运算
        findMacroInCondition(condition.expression, result);
        result.push(condition.operator);
    }
    else if (condition instanceof ug.AST_Call) {
        // macro
    ...
    else if (condition instanceof ug.AST_SymbolRef && condition.thedef.init) {
        // variable
        result.push(condition.thedef.init.value);
    }
    else {
        // primary value
